...
class JHS{
	constructor(options) {
		...
		this.latex = require('./execlatex-2')();
		this.queryString = "";
		this.echo_str = '\n\n'+
	'function echo(s){\n'+
	...
	'else str += s.toString();} cadena += str;}; return cadena;\n\n';
	}	
	...
	if(c){
		...
		c = c + this.echo_str;  
		 ...
		 ////// but 'fs' & 'vm' packages are excluded  //////////
		 ...
		 ////////////////////////////////////////////////////////
       	...
       	try{
					var script = new vm.Script(code);	
			 		...
			 		if(flags.katex){
			 			var html = Buffer.from(sandbox.jhs(),'utf-8').toString();
			 			this.latex.parseFile(html,true,function(result){
						callback(result, error);	
						});
			 		}
			 		else callback(sandbox.jhs(),error);
	...
	require_filter(pack){
	 //if(/*pack == 'fs'  || */ pack == 'vm') 
	 	...
	 	console.log({banned_require:pack});
	 	return {};
		}
    return require(pack);
